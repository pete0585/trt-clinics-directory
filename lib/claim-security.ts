...
export async function requestClaim(req: NextRequest) {
  try {
    ...
    const saved = await db.from(CLAIM_CONFIG.claims).insert({ listing_id: listingId, email: listing.data.email, token, verified: false, expires_at: new Date(Date.now() + 30 * 24 * 3600000).toISOString() }).select('id').single()
